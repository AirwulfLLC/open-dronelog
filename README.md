...
- **Thermal Studio (desktop)**: Import DJI radiometric JPEGs (R-JPEG from H20T/H30T/M30T/M3T/M2EA/M4T, etc.), photos, and videos. Per-pixel temperature analysis via the DJI Thermal SDK: palettes, adjustable scale, spot readout, min/max markers, user-defined temperature-range isolation (isotherm), and AI variance analysis that detects hot/cold anomaly regions with severity grading and defect classification. Annotate images (arrows, text, freehand, circles, boxes) and build inspection reports (header data, side-by-side visual/IR imaging log, anomaly log with geotags and T<sub>max</sub>/T<sub>min</sub>, action plan) exported as PDF or RTF. Includes a SINDA-style heat-flow network solver: place computation nodes on the image, connect them with linear (Fourier conduction/convection) or radiative (σ·εFA·ΔT⁴) conductors, apply time/temperature-varying sources and m·cp, and solve steady-state or transient — heat flow paths render as arrows on the image with per-node surface heat balances. Imports Agisoft Metashape exports (GeoTIFF orthomosaics/DEMs with in-app previews, processing reports, camera files, point clouds), links DJI flights to inspections, and saves complete inspections as single portable .odlbundle files. See [docs/thermal-analysis.md](docs/thermal-analysis.md) for SDK setup.
