...
export function SettingsModal({ isOpen, onClose }: SettingsModalProps) {
  ...
  const [thermalAiKey, setThermalAiKey] = useState('');
  const [thermalAiCfg, setThermalAiCfg] = useState<thermalApi.ThermalAiConfig | null>(null);
  ...
  const checkApiKey = async () => {
    ...
    // AI thermal analysis config (desktop only — thermal module is Tauri-only)
    if (!isWebMode()) {
      try {
        setThermalAiCfg(await thermalApi.thermalAiGetConfig());
      } catch (err) {
        console.error('Failed to load AI analysis config:', err);
      }
    }
  };
        ...
        <div className="flex-1 p-4 overflow-y-auto min-h-0 settings-scroll">
          <div className="flex flex-col lg:flex-row gap-6 lg:gap-0">
            ...
            <div className="lg:w-1/2 space-y-4 lg:pr-5">
              ...
              {/* AI Thermal Analysis Section (provider + per-provider API key) */}
              <div className="pt-4 border-t border-gray-700">
                <label className="block text-sm font-medium text-gray-300 mb-2">
                  {t('settings.thermalAiApiKey', 'AI Thermal Analysis')}
                </label>
                <p className="text-xs text-gray-500 mb-3">
                  {t(
                    'settings.thermalAiApiKeyDesc',
                    'Provider and API key used to generate AI narrative findings in the Thermal Studio. Keys are stored per profile on this device and only sent to the selected provider.',
                  )}
                </p>

                {(() => {
                  const AI_PROVIDERS: Array<{
                    value: thermalApi.ThermalAiProvider;
                    label: string;
                    keyUrl: string;
                    placeholder: string;
                  }> = [
                  ...
                  const cfg = thermalAiCfg;
                  const provider = cfg?.provider ?? 'claude';
                  const meta = AI_PROVIDERS.find((p) => p.value === provider) ?? AI_PROVIDERS[0];
                  const hasKeyFor = (p: thermalApi.ThermalAiProvider) =>
                    cfg
                      ? { claude: cfg.hasClaudeKey, openai: cfg.hasOpenaiKey, gemini: cfg.hasGeminiKey }[p]
                      : false;
                  const selectedHasKey = hasKeyFor(provider);
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-2 mb-3">
                        <span className="text-xs text-gray-400 whitespace-nowrap">
                          {t('settings.thermalAiProvider', 'Provider')}
                        </span>
                        <div className="flex-1 min-w-0">
                          <Select
                            ...
                            onChange={async (v) => {
                              try {
                                await thermalApi.thermalAiSetProvider(v as thermalApi.ThermalAiProvider);
                                setThermalAiKey('');
                                setThermalAiCfg(await thermalApi.thermalAiGetConfig());
                              } catch (err) {
                            ...
                            options={AI_PROVIDERS.map((p) => ({
                              value: p.value,
                              label: hasKeyFor(p.value) ? `${p.label} ✓` : p.label,
                            }))}
                          />
                        </div>
                      </div>

                      {/* Status for the selected provider */}
                      <div className="flex items-center gap-2 mb-3 flex-wrap">
                        <span className="text-sm text-gray-400">
                          {selectedHasKey ? t('settings.apiKeyConfigured') : t('settings.noApiKey')}
                        </span>
                        {selectedHasKey && (
                          <button
                            onClick={async () => {
                              try {
                                await thermalApi.thermalAiRemoveApiKey(provider);
                                setThermalAiCfg(await thermalApi.thermalAiGetConfig());
                                setMessage({
                                  type: 'success',
                                  text: t('settings.thermalAiKeyRemoved', 'AI analysis API key removed'),
                                });
                              } catch (err) {
                                setMessage({ type: 'error', text: `${t('settings.failedToRemoveKey')}: ${err}` });
                              }
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-xs text-drone-primary hover:underline"
                        >
                          {t('settings.thermalAiApiKeyGet', 'Get a key')}
                        </a>
                      </div>

                      {/* Key input for the selected provider */}
                      <div className="flex gap-2">
                        <PasswordInput
                          wrapperClassName="flex-1 min-w-0"
                          value={thermalAiKey}
                          onChange={(e) => setThermalAiKey(e.target.value)}
                          placeholder={selectedHasKey ? '••••••••••••••••' : meta.placeholder}
                          className="w-full px-3 py-1.5 bg-gray-900 border border-gray-700 rounded-lg text-xs text-gray-200 focus:outline-none focus:ring-1 focus:ring-drone-primary"
                        />
                        <button
                          onClick={async () => {
                            if (!thermalAiKey.trim()) {
                              setMessage({ type: 'error', text: t('settings.enterApiKey') });
                              return;
                            }
                            try {
                              await thermalApi.thermalAiSetApiKey(provider, thermalAiKey.trim());
                              setThermalAiKey('');
                              setThermalAiCfg(await thermalApi.thermalAiGetConfig());
                              setMessage({
                                type: 'success',
                                text: t('settings.thermalAiKeySaved', 'AI analysis API key saved'),
                              });
                            } catch (err) {
                              setMessage({ type: 'error', text: `${err}` });
                            }
                          }}
                          disabled={!thermalAiKey.trim()}
                          className="py-1.5 px-3 rounded-lg bg-drone-primary text-white text-xs hover:bg-drone-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors whitespace-nowrap"
                        >
                          {selectedHasKey ? t('settings.updateApiKey') : t('settings.saveApiKey')}
                        </button>
                      </div>
                    </>
                  );
            ...
            {/* Right Column: Support, Info & Data */}
